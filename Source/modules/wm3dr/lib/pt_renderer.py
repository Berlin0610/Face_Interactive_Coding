...
#     # be careful for translation
#     translation = self.decode_translation(translation, index)
#     #rot_mat, trans_xyz = self.set_transform_matrices(torch.cat([angles, translation], dim=-1))   
#     rot_mat, trans_xyz = get_transform_matrices(torch.cat([angles, translation], dim=-1))       
...
#     return rot_mat, trans_xyz
  ...
  def set_RotTransLight(self, angles, translation, index):
    # be careful for translation
    translation = self.decode_translation(translation, index)
    self.set_transform_matrices(torch.cat([angles, translation], dim=-1))
  ...
  def get_Landmarks(self, Shape):
    ...
    projection = Shape.bmm(K.transpose(2, 1))
    projection_ = projection[..., :2] / projection[..., 2:]
    projection_ = torch.stack((projection_[:,:,0], self.input_res - projection_[:,:,1]), 2)
    return projection_, projection_[:, self.BFM.keypoints, :]
  ...
  def Illumination_new(self, Albedo, canoShape, gamma):
    face_norm = self.Compute_norm(canoShape)
    # face_norm = face_norm[:2]
    face_norm_r = face_norm.bmm(self.rot_mat.transpose(2, 1))
    face_color, lighting = self.Illumination_layer(Albedo, face_norm_r, gamma)
    return face_color, lighting


  def Projection_layer(self, face_shape,rotation,translation,focal=1015.0,center=128.0): 
    focal = 1015.0
    center = 128.0
    cam_pos = 10
    ...
    face_shape_t[:, :, 2] = cam_pos - face_shape_t[:, :, 2]

    aug_projection = face_shape_t.bmm(p_matrix.permute(0, 2, 1))

    face_projection = aug_projection[:, :, 0:2] / aug_projection[:, :, 2:]
    #face_projection = torch.stack((face_projection[:,:,0], 256 - face_projection[:,:,1]), 2)

    # # CHJ_WARN: I do this for visualization
    # z_buffer = _need_const.cam_pos - aug_projection[:, :, 2:] # CHJ: same as the z of  face_shape_t
    
    #face_projection =  face_shape_t[..., :2] / face_shape_t[..., 2:]
    landmark_2d = face_projection[:, self.BFM.keypoints, :]
    
    
    return face_shape_t, face_projection, landmark_2d
