...
def make_prediction(reference_frame, source_reconstruction, driving_reconstruction, generator):
    ...
    with torch.no_grad(): # 不追踪梯度信息，加速推理
...
#      
                        reference_reconstruction.update({ 'rendered_source':source_rendered_reconstruct.permute(0,3,1,2), 
                                                         'eyemask_source':source_eyemask_reconstruct.permute(0,3,1,2)})  
                    ...
                    elif  exp_dim==12:   #else:
                        kp_integer,exp_value,angle_value,tranlation_value,inds_value,au45_value=listformat_3dmm_12(kp_previous, kp_difference) 
                        seq_kp_integer.append(kp_integer)                             
                    ...
                    driving_reconstruction.update({'rendered_driving':driving_rendered_reconstruct.permute(0,3,1,2),
                                                   'eyemask_driving':driving_eyemask_reconstruct.permute(0,3,1,2)})         
